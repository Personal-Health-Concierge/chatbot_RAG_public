...
// @ts-ignore
import YoutubeVideos from "../medias/youtube"
...
export default function ChatMessage(props: ChatMessageProps) {
  ...
  const charts: any = metadata ? metadata?.charts : null
  const medias: any = metadata ? metadata?.medias : null
      ...
      {props.message?.type === ChatBotMessageType.AI && (
        ...
        >
...

          {
            ...
            charts.map((x: any, i: number) => (
              <LineChart key={i}
                chartTitle={x.chartTitle}
                chartData={x.chartData}
              />
            ))
            */
            <LineChart
              title="LIPID Levels Over Time"
              data={charts}
            />
          }
          {
            medias && (medias.length > 0) &&
            (<YoutubeVideos urls={medias} />)
          }
